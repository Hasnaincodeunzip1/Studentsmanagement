...
from datetime import datetime, timedelta
from notifications.utils import notify_course_hold_request, notify_course_hold_decision
from notifications.utils import notify_new_study_material
from notifications.utils import notify_trainer_assignment
...
class CourseHoldViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        course_hold = serializer.save(requested_by=self.request.user)
        notify_course_hold_request(course_hold)
    ...
    def approve(self, request, pk=None):
        ...
        notify_course_hold_decision(course_hold)
...
class StudentCourseViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role in ['ADMIN', 'MANAGER']:
            ...
            return StudentCourse.objects.filter(student=user, end_date__gte=today)
...
class StudyMaterialViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        study_material = serializer.save(created_by=self.request.user)
        notify_new_study_material(study_material)
...
class StudentFeedbackViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        feedback = serializer.save(student=self.request.user)
        notify_admins_and_managers(
            'FEEDBACK_SUBMISSION',
            f"New feedback submitted by {feedback.student.username} for course {feedback.course.name}"
        )
    ...
    def respond(self, request, pk=None):
        ...
        if status_value not in dict(StudentFeedback.STATUS_CHOICES):
...
class TrainerAssignmentViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        notify_trainer_assignment(assignment.trainer, assignment.course)

...
def check_course_end_dates():
    ...
    for course in ending_courses:
        ...
        # Unassign student when course ends
        if days_left == 0:
            if not course.course.is_group_class:
                course.trainer = None
            course.save()

            # Additional notification for course end
            notify_admins_and_managers(
                'COURSE_ENDED',
                f"Course for student {course.student.username} (ID: {course.student.id}) has ended and student has been unassigned."
            )

            create_notification(
                course.student,
                'COURSE_ENDED',
                f"Your course {course.course.name} has ended. You have been unassigned from the course."
            )

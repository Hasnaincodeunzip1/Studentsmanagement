...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def search_by_email(self, request):
        ...
        users = self.get_queryset().filter(email__iexact=email)
        
        # Check if the requesting user has permission to view these users
        users = [user for user in users if self.request.user.has_perm('view_user', user)]
        
        if not users:
            return Response({"message": "No users found with this email"}, status=status.HTTP_404_NOT_FOUND)

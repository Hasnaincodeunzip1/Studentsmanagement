...
from .models import BigBlueButtonRecording, BigBlueButtonRoom
from .serializers import BigBlueButtonRecordingSerializer, BigBlueButtonRecording, BigBlueButtonRoomSerializer, UserRoomSerializer
from .tasks import sync_bbb_recordings
from .filters import BigBlueButtonRecordingFilter
...
class BigBlueButtonRoomViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
     ...
     if user.role in ['ADMIN', 'MANAGER']:
        logger.info(f"User {user.username} is ADMIN/MANAGER. No additional filtering.")
     elif user.role == 'TRAINER':
        logger.info(f"Filtering queryset for TRAINER {user.username}")
        queryset = queryset.filter(
            Q(room__course__trainers=user) | Q(room__student_course__trainer=user)
        ).distinct()
     else:  # STUDENT
        logger.info(f"Filtering queryset for STUDENT {user.username}")
        queryset = queryset.filter(
...
class BigBlueButtonRecordingViewSet(viewsets.ModelViewSet):
    ...
    def sync_recordings(self, request):
        task = sync_bbb_recordings.delay()
        return Response({
            "message": "Sync task initiated",
            "task_id": task.id
        })

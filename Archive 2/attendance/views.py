...
class AttendanceViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if student_course.end_date < today:
            logger.warning(f"Attendance creation attempt for ended course by user {request.user.username}")
            return Response({"detail": "Cannot mark attendance for ended course."}, 
                            status=status.HTTP_400_BAD_REQUEST)
